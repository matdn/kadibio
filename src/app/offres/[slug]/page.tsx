...
const offres = {
    ...
    entreprise: {
        ...
        description: "Régalez vos équipes lors de déjeuners pros, séminaires ou événements d’entreprise avec notre cuisine engagée et locale.",
        plus: "Nos plats sont conçus pour être équilibrés, savoureux et faciles à servir. Une pause gourmande et responsable.",
    },
    anniversaires: {
        title: "Anniversaires & Moments de vie",
        image: "/images/crumble.jpg",
        description: "Fêtez vos moments les plus précieux avec une cuisine généreuse, maison et pleine de fraîcheur.",
        plus: "Des menus adaptés à tous les âges, à toutes les préférences et à toutes les saisons. Et toujours avec des produits bio.",
    },
};

export async function generateStaticParams() {
    return Object.keys(offres).map((slug) => ({ slug }));
}

export default async function OffrePage({ params }: { params: Promise<{ slug: string; }>; }) {
    const { slug } = await params; // ATTENTION à faire "await params"

    const offre = offres[slug as keyof typeof offres];
        ...
        <main className="bg-[#B3C7D6] min-h-screen py-16 px-4 flex flex-col items-center text-white">
            <div className="w-full max-w-6xl bg-white rounded-xl overflow-hidden text-black shadow-xl">
                ...
                <div className="w-full h-[400px] relative">
                    <Image src={offre.image} alt={offre.title} fill className="object-cover" />
                </div>
